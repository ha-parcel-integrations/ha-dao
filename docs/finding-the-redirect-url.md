...
During setup, DAO's sign-in page finishes by redirecting your browser to a URL
that starts with `daoapp://auth-callback`. No desktop browser has an app
registered for that address, so **the sign-in will visibly fail or hang at the
last step** — you'll see a blank page, an error, or a spinner that never
resolves. That is expected, not a bug. Home Assistant needs that failed URL,
not a successful page load, so you have to catch it with your browser's
developer tools before it disappears.
...
2. Turn on **"preserve log"** (the exact wording differs per browser — see
   ...
   redirects.
3. Complete the DAO sign-in normally.
4. When the page fails to continue, look through the Network tab's request
   list for one whose **name/URL starts with `daoapp://auth-callback`**.
5. Copy that request's **full URL** — not just its name as shown in the list,
   the complete address including everything after `?code=`, and the
   `state=` parameter after it.
...
5. Go back to the page and complete the DAO sign-in.
6. When the flow stops/errors, look at the request list in the Network panel.
   ...
   one beginning with `daoapp:`. You can also type `daoapp` into the
...
4. Complete the DAO sign-in.
5. Filter for `daoapp` in the request list, click the matching request.
...
4. Complete the DAO sign-in.
5. Type `daoapp` into the Network panel's filter box to find the request.
...
4. Look for a **"Preserve Log"** button/checkbox in the Network tab's toolbar
   and enable it.
5. Complete the DAO sign-in.
6. Look through the request list for one starting with `daoapp:` — Safari's
   filter field at the top of the Network tab also accepts `daoapp` as a
   search term.
...
Mobile browsers don't have on-device developer tools with a Network tab, and
if the DAO app is installed the phone will hand the `daoapp://` link straight
to the app instead of leaving it visible in a browser at all. Complete the
sign-in on a **desktop/laptop browser** instead, using the steps above — the
redirect URL isn't tied to a specific device, so where you catch it doesn't
matter, only that Home Assistant is the one you paste it into afterwards.

## Troubleshooting

- **I don't see any request starting with `daoapp:`.** Make sure "Preserve
...
- **The address I copied gets rejected by Home Assistant.** Paste the request
  ...
  (`?code=…&state=…`). A partial copy (e.g. just the `code=` value, without
  the rest of the address) is rejected up front rather than silently
  accepted — DAO's config flow checks the pasted `state` against the one it
  generated before ever exchanging the code, and a bare code carries no
  `state` to check. The code itself is single-use and short-lived — if it's
  been more than a couple of minutes, redo the sign-in and copy the new one
  rather than retrying the old value.
- **The request never appears at all, the page just shows a normal DAO
  error.** Confirm you're actually being redirected past the login step — an
  incomplete or failed sign-in won't reach the `daoapp://auth-callback` step
  at all, so there's nothing to catch yet.
- **Never share the redirect URL, authorization code, or token in a public
  issue** — anyone with the full address can complete the login on your
  behalf while the code is still valid.
