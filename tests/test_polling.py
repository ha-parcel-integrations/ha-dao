...
from custom_components.dao.api import DAOApiError
...
from custom_components.dao.coordinator import (
    DAOCoordinator,
...
from .payloads import in_transit_sample
...
async def test_update_interval_is_mid_for_an_in_transit_parcel(hass):
    """DAO can never report `out_for_delivery` (see parcels.py) — a real
    active parcel therefore always lands in the mid tier, never hot."""
    entry = _entry()
    entry.add_to_hass(hass)
    client = AsyncMock()
...
async def test_429_raises_update_failed_with_retry_after(hass):
    ...
    entry.add_to_hass(hass)
    client = AsyncMock()
    client.async_get_parcels.side_effect = DAOApiError(
