"""Tests for DAO sensor property logic."""
from datetime import datetime, timezone
from unittest.mock import MagicMock

from custom_components.dao.const import ParcelStatus
from custom_components.dao.sensor import (
    DAOAwaitingPickupSensor,
    DAODeliveredParcelsSensor,
...
def test_incoming_counts_and_lists():
    ...
    sensor = DAOIncomingParcelsSensor(coordinator, _entry(), lambda _: None, set())
...
def test_parcel_sensor_status_and_attributes():
    ...
    sensor = DAOParcelSensor(_coordinator([parcel]), _entry(), "A")
...
def test_parcel_sensor_missing_barcode():
    sensor = DAOParcelSensor(_coordinator([_parcel("A")]), _entry(), "OTHER")
...
def test_next_delivery_picks_earliest():
    ...
    sensor = DAONextDeliverySensor(coordinator, _entry())
...
def test_next_delivery_none_without_moments():
    sensor = DAONextDeliverySensor(_coordinator([_parcel("A")]), _entry())
    assert sensor.native_value is None
    assert sensor.extra_state_attributes == {}
...
def test_awaiting_pickup_counts_only_at_pickup_point():
    ...
    sensor = DAOAwaitingPickupSensor(coordinator, _entry())
...
def test_delivered_sensor():
    ...
    sensor = DAODeliveredParcelsSensor(coordinator, _entry())
    assert sensor.native_value == 1
